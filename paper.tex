...
\begin{abstract}
Detecting the nine-dash line in real-world imagery is challenging because the target is often small, fragmented, and visually similar to ordinary cartographic strokes. We introduce a moderation-oriented benchmark containing 7,344 images and 2,636 annotated objects, including positive images, in-domain map-like negatives, and out-of-domain negatives. Faster R-CNN, SSD, and recent YOLO detectors are evaluated using both COCO-style detection metrics and image-level false-positive measures. We further propose HN-SARD, a Hard-Negative and Scale-Aware Region Distillation framework that aligns positive Faster R-CNN RoIs with frozen DINOv2 context-crop embeddings while separating foreground-like negative RoIs from positive teacher anchors. Training with both negative groups reduces FPR@95TPR from 0.088 to 0.012 and negative FPPI from 0.889 to 0.096. Relative to its Faster R-CNN base, HN-SARD improves AP from 0.685 to 0.713 and R@1\%FPR from 0.940 to 0.960, while reducing negative FPPI to 0.055. Although modern one-stage detectors may achieve higher overall AP, the results demonstrate the importance of explicit hard negatives and moderation-oriented evaluation for reliable cartographic-symbol screening.
\end{abstract}
...
Hard-negative methods include online mining \cite{shrivastava2016ohem}, loss re-weighting \cite{lin2017focal}, and classification refinement \cite{cheng2018dcr}. Outlier Exposure likewise trains against auxiliary outliers to improve anomaly rejection \cite{hendrycks2019oe}. These approaches emphasize difficult background rather than uniformly sampling easy negatives. Because high AP can coexist with many false alarms, we make in-domain and out-of-domain negatives explicit training and evaluation groups and report both threshold-dependent image errors and the number of false boxes.
...
HN-SARD follows teacher--student and detector distillation \cite{hinton2015distilling,li2022rank}, but uses a frozen self-supervised teacher on region crops and explicitly emphasizes hard negatives \cite{shrivastava2016ohem,cheng2018dcr}. Detector distillation has transferred instance, localization, contrastive, and scale-aware knowledge \cite{dai2021gid,zheng2022localization,yao2021gdetkd,zhu2023scalekd}. DINO and DINOv2 provide strong general-purpose visual representations \cite{caron2021dino,oquab2023dinov2}. Previous work employed Faster R-CNN with DINOv2-based knowledge distillation for nine-dash-line detection~\cite{tran2025detecting}. Our region-level design differs from full-image distillation: resizing a matched context crop to the teacher input gives small symbolic details more pixels while retaining nearby map evidence. The teacher supplies only an auxiliary training signal and does not replace detection supervision.
...
Validation controls checkpoint selection, early stopping, and all HN-SARD hyperparameters. The final configuration is frozen before test evaluation. Main comparisons and loss sensitivity are reported on the held-out test split, whereas context scale is evaluated on validation because it is a training hyperparameter. All predictions are processed by the same evaluator using the fixed split and seed.

\subsection{Baseline Comparison}
Table~\ref{tab:baselines} compares held-out test performance; it is not a speed benchmark or exhaustive leaderboard.

YOLOv9s achieves the highest AP and AR, while YOLOv9s and YOLOv10s yield the lowest negative FPPI. HN-SARD does not surpass the strongest YOLO models in overall AP, but improves AP, AP$_t$, FPR@95TPR, fixed-budget recall, and FPPI over its Faster R-CNN base. Its image AUROC of 0.996 is also the highest in the comparison. These results position YOLOv9s as the strongest general detector, while the Faster R-CNN pair provides the direct controlled evaluation of HN-SARD.
...
The results show that explicit negative data is essential for
moderation-oriented detection: it changes AP only slightly but
substantially reduces FPR and FPPI, particularly on map-like
in-domain negatives. Model selection should therefore consider
group-specific false-positive metrics and review-budget recall in
addition to object AP. The threshold $t=0.25$ is used only for
diagnostic comparison; deployment thresholds should be selected
on validation data. HN-SARD improves its Faster R-CNN parent
without inference-time teacher cost, but it does not outperform
the strongest YOLO baselines in overall AP and has only been
evaluated with R50-FPN.

The benchmark also has limitations. Validation and test contain
few tiny instances, so tiny AP is a diagnostic rather than a
general tiny-object claim. Bounding boxes do not describe
individual-dash geometry, and the collected data may not cover
future shifts in map style, language, compression, and publishing
source. Future work should study source-disjoint evaluation,
higher-resolution training, confidence intervals for tiny AP, and
region correspondence mechanisms for dense detectors.
...
S. Ren, K. He, R. Girshick, and J. Sun, ``Faster R-CNN: Towards real-time object detection with region proposal networks,'' in \emph{Advances in Neural Information Processing Systems}, vol. 28, 2015, pp. 91--99.

\bibitem{lin2017fpn}
T.-Y. Lin, P. Doll\'ar, R. Girshick, K. He, B. Hariharan, and S. Belongie, ``Feature pyramid networks for object detection,'' in \emph{Proc. CVPR}, 2017, pp. 2117--2125.

\bibitem{liu2016ssd}
W. Liu \emph{et al.}, ``SSD: Single shot MultiBox detector,'' in \emph{Proc. ECCV}, 2016, pp. 21--37.

\bibitem{redmon2016you}
J. Redmon, S. Divvala, R. Girshick, and A. Farhadi, ``You only look once: Unified, real-time object detection,'' in \emph{Proc. CVPR}, 2016, pp. 779--788.

\bibitem{jocher2023yolov8}
G. Jocher, A. Chaurasia, and J. Qiu, ``Ultralytics YOLOv8,'' software, version 8.0.0, 2023. [Online]. Available: \url{https://github.com/ultralytics/ultralytics}. [Accessed: Jul. 22, 2026].

\bibitem{wang2024yolov9}
C.-Y. Wang, I.-H. Yeh, and H.-Y. M. Liao, ``YOLOv9: Learning what you want to learn using programmable gradient information,'' in \emph{Proc. ECCV}, 2024, pp. 1--21.

\bibitem{wang2024yolov10}
A. Wang \emph{et al.}, ``YOLOv10: Real-time end-to-end object detection,'' in \emph{Advances in Neural Information Processing Systems}, vol. 37, 2024, pp. 107984--108011.

\bibitem{ultralytics2024yolo11}
G. Jocher and J. Qiu, ``Ultralytics YOLO11,'' software, version 11.0.0, 2024. [Online]. Available: \url{https://github.com/ultralytics/ultralytics}. [Accessed: Jul. 22, 2026].

\bibitem{kisantal2019augmentation}
M. Kisantal, Z. Wojna, J. Murawski, J. Naruniec, and K. Cho, ``Augmentation for small object detection,'' in \emph{Proc. 9th Int. Conf. Advances in Computing and Information Technology (ACITY)}, 2019, pp. 119--133.

\bibitem{shrivastava2016ohem}
A. Shrivastava, A. Gupta, and R. Girshick, ``Training region-based object detectors with online hard example mining,'' in \emph{Proc. CVPR}, 2016, pp. 761--769.

\bibitem{lin2017focal}
T.-Y. Lin, P. Goyal, R. Girshick, K. He, and P. Doll\'ar, ``Focal loss for dense object detection,'' in \emph{Proc. ICCV}, 2017, pp. 2980--2988.

\bibitem{cheng2018dcr}
B. Cheng \emph{et al.}, ``Decoupled classification refinement: Hard false positive suppression for object detection,'' arXiv preprint arXiv:1810.04002, 2018.

\bibitem{hendrycks2019oe}
D. Hendrycks, M. Mazeika, and T. Dietterich, ``Deep anomaly detection with outlier exposure,'' in \emph{Proc. ICLR}, 2019.

\bibitem{hinton2015distilling}
G. Hinton, O. Vinyals, and J. Dean, ``Distilling the knowledge in a neural network,'' arXiv preprint arXiv:1503.02531, 2015.

\bibitem{li2022rank}
G. Li, X. Li, Y. Wang, S. Zhang, Y. Wu, and D. Liang, ``Knowledge distillation for object detection via rank mimicking and prediction-guided feature imitation,'' in \emph{Proc. AAAI Conf. Artif. Intell.}, vol. 36, no. 2, 2022, pp. 1306--1313.

\bibitem{dai2021gid}
X. Dai, Z. Jiang, Z. Wu, Y. Bao, Z. Wang, S. Liu, and E. Zhou, ``General instance distillation for object detection,'' in \emph{Proc. CVPR}, 2021, pp. 7842--7851.

\bibitem{zheng2022localization}
Z. Zheng, R. Ye, P. Wang, D. Ren, W. Zuo, Q. Hou, and M.-M. Cheng, ``Localization distillation for dense object detection,'' in \emph{Proc. CVPR}, 2022, pp. 9407--9416.

\bibitem{yao2021gdetkd}
L. Yao, R. Pi, H. Xu, W. Zhang, Z. Li, and T. Zhang, ``G-DetKD: Towards general distillation framework for object detectors via contrastive and semantic-guided feature imitation,'' in \emph{Proc. ICCV}, 2021, pp. 3591--3600.

\bibitem{zhu2023scalekd}
Y. Zhu, Q. Zhou, N. Liu, Z. Xu, Z. Ou, X. Mou, and J. Tang, ``ScaleKD: Distilling scale-aware knowledge in small object detector,'' in \emph{Proc. CVPR}, 2023, pp. 19723--19733.

\bibitem{caron2021dino}
M. Caron \emph{et al.}, ``Emerging properties in self-supervised vision transformers,'' in \emph{Proc. ICCV}, 2021, pp. 9650--9660.

\bibitem{oquab2023dinov2}
M. Oquab \emph{et al.}, ``DINOv2: Learning robust visual features without supervision,'' \emph{Trans. Mach. Learn. Res.}, 2024.

\bibitem{tran2025detecting}
T. D. Tran, H. M. Danh, and L. D. Hoang, ``Detecting `Nine-Dash Line' images in digital content via Faster R-CNN and DINOv2-based knowledge distillation,'' in \emph{Proc. 2025 RIVF Int. Conf. Comput. Commun. Technol. (RIVF)}, Ho Chi Minh City, Vietnam, 2025, pp. 322--327, doi: 10.1109/RIVF68649.2025.11365135.
